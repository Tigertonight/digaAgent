...
describe("Workbench overview model", () => {
  ...
  it("keeps Agent Team simple by default while preserving advanced controls", () => {
    ...
    expect(source).toContain("团队现在在做什么");
    expect(source).toContain("需要你处理");
    expect(source).toContain("查看团队过程");
    expect(source).toContain("detailsOpen");
    expect(source).toContain("diagnosticsOpen");
    ...
    expect(source).toContain("最近执行");
    expect(source).toContain("teamEventUserText");
    expect(source).toContain("接下来团队会做什么");
    ...
    expect(source).toContain("模型判断");
    expect(source).toContain("最终综合");
    expect(source).toContain("诊断详情");
    ...
    expect(source).toContain("负责人会自动分配任务");
    expect(source).toContain("不需要你手动调度成员");
    expect(source).toContain("等待负责人自动分配下一步");
    expect(source).toContain("待安排");
  ...
  it("keeps Agent Team launch explicit with a confirmation panel", () => {
    ...
    expect(source).toContain("准备启动团队协作");
    expect(source).toContain("推荐配置");
    expect(source).toContain("已根据你的任务自动选择，通常不用调整。");
    expect(source).toContain("teamLaunchAdvancedOpen");
    expect(source).toContain("只看不改");
    expect(source).toContain("可以帮我改");
    expect(source).toContain("需要查资料");
    expect(source).toContain("互相校验结论");
    ...
    expect(source).toContain("只有你确认后才会启动");
