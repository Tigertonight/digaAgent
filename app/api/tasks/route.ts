...
export async function GET(req: Request) {
  ...
  if (auth) return auth;
  return NextResponse.json(listLongTasksDashboard());
}
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (type === "create") {
      ...
      return NextResponse.json({ ok: true, task, dashboard: listLongTasksDashboard() });
    }
    ...
    if (type === "delete") {
      const id = stringField(body.id);
      deleteLongTask(id);
      return NextResponse.json({ ok: true, dashboard: listLongTasksDashboard() });
    }

    if (type === "run") {
      const id = stringField(body.id);
      const result = await startLongTaskRun(id);
      return NextResponse.json({ ok: true, ...result, dashboard: listLongTasksDashboard() });
    }
    ...
    if (type === "finding_status") {
      ...
      return NextResponse.json({ ok: true, finding, dashboard: listLongTasksDashboard() });
    }
...
async function startLongTaskRun(taskId: string): Promise<{
...
function attachRunLifecycle(agentId: string, runId: string, taskId: string): void {
  ...
  const handleEvent = (event: { type?: string; message?: unknown }) => {
    if (event.type === "approval_request") {
      ...
      finalText = messageText((event as { message?: unknown }).message) || finalText;
...
function messageText(message: unknown): string {
  ...
  return content
    .map((part) => {
      ...
      const rec = part as { type?: string; text?: string; thinking?: string };
...
function buildLongTaskPrompt(task: LongTaskDefinition): string {
  return [
    "你正在以 Diga 长期任务模式运行。请把自己当成一个会长期替用户盯事、跑事、汇报事、等待用户决策的工作系统。",
    "",
    `任务名称：${task.title}`,
